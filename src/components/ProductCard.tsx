...
const ProductCard = ({ product }: Props) => {
  ...
  return (
    <div
      ...
      className={`max-w-xs border-r transition-all duration-300 ${
        isHovered ? 'shadow-[0px_-5px_40px_-15px_rgba(0,0,0,0.3)]' : ''
      }`}
    >
        ...
        <div className="flex justify-end">
          <FaRegHeart
            className={`cursor-pointer transition-all duration-300 hover:text-red-500 ${
              isHovered ? '-translate-x-5' : 'opacity-0 translate-x-3'
            }`}
          />
        </div>
